...
sbatch --array=1-$joblength trim_align_merge.sh -i -u -q \
-R ${reference_genome} \
-M ${mito_genome} \
-O ${working_directory}/bams
```

## Call GLs on each chunk with angsd
...
cd ${working_directory}
dos2unix make_gls_angsd_chunked_filt.sh
dos2unix chr_intervals.txt

cat chr_intervals.txt > angsd_job_index.txt
joblength=$(cat angsd_job_index.txt | wc -l)

sbatch --array=1-$joblength make_gls_angsd_chunked_filt.sh \
-R ${reference_genome} \
-M ${working_directory}/manifest.txt \
-I ${working_directory}/bams/bams \
-O ${working_directory}/angsdfilt \
-q 20 \
-m 20 
```
